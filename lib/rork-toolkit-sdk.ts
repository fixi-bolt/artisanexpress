...
const API_BASE = process.env.EXPO_PUBLIC_TOOLKIT_URL;

function getBaseUrl(): string {
  const base = API_BASE ?? '';
  if (!base) {
    console.log('[toolkit-sdk] Missing EXPO_PUBLIC_TOOLKIT_URL env var');
  }
  return base;
}
...
export async function generateText(params: string | { messages: (UserMessage | AssistantMessage)[] }): Promise<string> {
  try {
    ...
    const res = await fetch(new URL('/agent/chat', getBaseUrl()).toString(), {
      method: 'POST',
