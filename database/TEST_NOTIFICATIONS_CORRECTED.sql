-- ========================================
-- 🧪 SCRIPT DE TEST DES NOTIFICATIONS (CORRIGÉ)
-- ========================================
-- Ce script :
...
BEGIN
    ...
    -- Supprimer les missions de test (clients.id = users.id)
    DELETE FROM missions 
    WHERE client_id IN (
        SELECT id FROM users WHERE email LIKE '%test-notif%' AND user_type = 'client'
    );
    
    -- Supprimer les artisans de test
    DELETE FROM artisans 
    WHERE id IN (
        SELECT id FROM users WHERE email LIKE '%test-notif%' AND user_type = 'artisan'
    );
    ...
    -- Créer le profil client (id = user_id)
    ...
    -- Créer le profil artisan (id = user_id)
...
SELECT 
    '  • ' || COALESCE(n.title, 'Sans titre') as notification,
    '  💬 ' || n.message as message,
    '  👁️ ' || CASE WHEN n.read THEN 'Lu' ELSE 'Non lu' END as statut,
