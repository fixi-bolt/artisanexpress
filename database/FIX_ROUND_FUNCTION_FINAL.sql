-- ═══════════════════════════════════════════════════════════════
-- 🔧 CORRECTION FINALE - FONCTION ROUND ET CRÉATION DE MISSION
-- ═══════════════════════════════════════════════════════════════
-- Ce script corrige l'erreur "function round(double precision, integer) does not exist"
-- lors de la création de missions
-- ═══════════════════════════════════════════════════════════════
...
-- 2️⃣ RECRÉER LA FONCTION CORRIGÉE (sans utiliser round)
-- ═══════════════════════════════════════════════════════════════
...
BEGIN
  ...
  LOOP
    ...
    IF distance_km <= COALESCE(artisan_record.intervention_radius, 50) THEN
      ...
      -- ✅ CORRECTION: Convertir distance_km en NUMERIC avant de l'utiliser dans le message
      ...
      VALUES (
        ...
        -- ✅ CORRECTION: Utiliser CAST pour convertir en NUMERIC, puis formater
...
-- Puis testez en créant une nouvelle mission
-- ═══════════════════════════════════════════════════════════════
